...
################################## teacher network ##################################
        ...
        self.models["encoder_teacher"] = networks.ResnetEncoder(
            self.opt.num_layers, self.opt.weights_init == "pretrained")
        
        self.models["depth_teacher"] = networks.DepthDecoder(
            self.models["encoder_teacher"].num_ch_enc, self.opt.scales)
        
        encoder_path = os.path.join(self.opt.teacher_weights_folder, "encoder.pth")
        decoder_path = os.path.join(self.opt.teacher_weights_folder, "depth.pth")
        ...
        model_dict = self.models["encoder_teacher"].state_dict()
        dec_model_dict = self.models["depth_teacher"].state_dict()
        
        self.models["encoder_teacher"].load_state_dict({k: v for k, v in encoder_dict.items() if k in model_dict})
        self.models["depth_teacher"].load_state_dict({k: v for k, v in decoder_dict.items() if k in dec_model_dict})
...
################################### student network #################################
        ...
        ### independent photometric uncertainty modelling:
        
        self.models["ph_uncert_enc"] = networks.ResnetEncoder(
                self.opt.num_layers,
                self.opt.weights_init == "pretrained",
                num_input_images=2)#num_input_images=2
        
        self.models["ph_uncert_dec"] = networks.UncertDecoder(
                self.models["pose_encoder"].num_ch_enc)

        self.models["ph_uncert_enc"].cuda()
        self.parameters_to_train += list(self.models["ph_uncert_enc"].parameters())
        self.models["ph_uncert_dec"].cuda()
        self.parameters_to_train += list(self.models["ph_uncert_dec"].parameters())
        ...
        self.num_total_batch = (train_dataset.__len__()) // self.opt.batch_size
    ...
    def process_batch_ts(self, inputs, outputs, losses):
        ### compute the pseudo ground truth
        for key, ipt in inputs.items():#inputs.values() has :12x3x196x640.
            inputs[key] = ipt.to(self.device)#put tensor in gpu memory
        outputs_t = self.models["depth_teacher"](self.models["encoder_teacher"](inputs["color_aug", 0 , 0]))
        
        for key, item in outputs_t.items():
            outputs_t[key].detach()
        regression_loss = self.regress_loss(outputs_t,outputs)
    ...
    def process_batch(self, inputs):
        ...
        losses = self.process_batch_ts(inputs, outputs, losses)
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    if f_i < 0:
                        ...
                        uncerts_inputs = [pose_feats[f_i], pose_feats[0]]
                    else:
                        pose_inputs = [pose_feats[0], pose_feats[f_i]]
                        uncerts_inputs = [pose_feats[0], pose_feats[f_i]]
                    ...
                    uncert_inputs = self.models["ph_uncert_enc"](torch.cat(uncerts_inputs,1))
                    uncert = self.models[("ph_uncert_dec")](uncert_inputs)
                    uncerts.append(uncert)
            photometric_uncerts = torch.cat(uncerts, 1)
            outputs["photometric_uncert"] = photometric_uncerts.mean(1,True)
...
#####################################################################
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            to_optimise_1 = to_optimise / outputs["photometric_uncert"] + torch.log(outputs["photometric_uncert"])
            loss += to_optimise_1.mean()
    ...
    def save_model(self):
        ...
        for model_name, model in self.models.items():
            if model_name in ["encoder", "depth", "uncert","pose_encoder","pose","ph_uncert_enc","ph_uncert_dec"]:
